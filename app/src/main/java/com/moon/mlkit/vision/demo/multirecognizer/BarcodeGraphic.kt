...
class BarcodeGraphic constructor(
  ...
  private val barcodeContext: String?
) : Graphic(overlay) {
  ...
  override fun draw(canvas: Canvas) {
    ...
    var textWidth = barcodePaint.measureText(barcode.displayValue)
    textWidth = max(
      textWidth,
      barcodePaint.measureText(barcodeContext)
    )
    canvas.drawRect(
      rect.left - STROKE_WIDTH,
      rect.top - 2 * lineHeight,
    ...
    canvas.drawText(barcodeContext!!, rect.left, rect.top - STROKE_WIDTH - lineHeight, barcodePaint)
  }
